#+title: An Incremental Approach to Compiler Construction (Abdulaziz Ghuloum)
...
我在wiki上看到了作者也是 Ikarus（scheme编译器）的作者 [[https://en.wikipedia.org/wiki/Ikarus_(Scheme_implementation)][Ikarus (Scheme implementation) - Wikipedia]].
导师是大名鼎鼎的 [[https://en.wikipedia.org/wiki/R._Kent_Dybvig][R. Kent Dybvig - Wikipedia]]. 我对Dybvig的所有了解都是来自于王垠的文章 [[http://www.yinwang.org/blog-cn/2013/03/28/chez-scheme][Chez Scheme 的传说]]

这篇论文有一个更加详细的教程和代码，但是我没有时间阅读, [[https://github.com/namin/inc][namin/inc: an incremental approach to compiler construction]], 所以只是简单地看了一些这篇论文。我觉得这篇论文以及参考文献可以作为索引，一些基本的编译器实现问题可以在这里找到答案。
...
我们编译器的寄存器约定是：
- %esi 存放生成的closure对象包括：
  - 代码地址
  - free variabiles的地址或者是值
- %edi 存放当前正在被调用的closure对象.
...
[[../images/compiler-handle-tail-call.png]]
...
[[../images/compiler-cdecl-placement.png]]
