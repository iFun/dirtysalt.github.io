...
LCA可以使用 [[https://www.geeksforgeeks.org/tarjans-off-line-lowest-common-ancestors-algorithm/][Tarjan]] 的离线 [[file:codes/misc/random/tarjan_lca.py][算法]]，也可以将这个问题转变成为RMQ. 这个过程并不复杂，但是还比较有启发性。

以下图为例说明这个过程，假设我们需要找到4和9的LCA(2).

[[../images/find-lca-using-rmq-example.png]]

首先对这个图进行欧拉遍历(Euler Tour)：以类似preorder的方式进行DFS，在进出的时候都记录一次。
不仅记录节点(euler_tour_array)，还记录这个节点所在的depth(depth_array). 得到的结果如下图：

[[../images/find-lca-using-rmq-eulertour.png]]
...
[[../images/find-lca-using-rmq-fo.png]]
